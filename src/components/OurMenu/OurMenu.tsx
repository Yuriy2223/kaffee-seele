    ...
    <section id="menu" className="bg-cream/20">
      <Container className="px-4 py-10">
        <div className="text-center mb-10">
          <h3
            className="text-4xl md:text-5xl font-serif font-bold text-warm-brown mb-6"
            data-testid="menu-title"
          >
            Наше меню
          </h3>
          <p
            className="text-xl text-dark-text max-w-2xl mx-auto"
        ...
        <div className="mb-14">
          <h4
            className="text-3xl font-serif font-semibold text-sage-green mb-12 text-center"
            data-testid="coffee-menu-title"
          >
            ...
            {coffeeMenu.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  <h5
                    className="text-xl font-serif font-semibold text-warm-brown mb-2"
                    data-testid={`coffee-name-${index}`}
                  >
                    {item.name}
                  </h5>
                  <p
                    className="text-dark-text mb-4"
                    data-testid={`coffee-description-${index}`}
                  >
                    {item.description}
                  </p>
                  <div className="flex justify-between items-center">
                    <span
                      ...
                      data-testid={`coffee-price-${index}`}
                    >
                      ₴{item.price}
                    </span>
        ...
        <div>
          <h4
            className="text-3xl font-serif font-semibold text-sage-green mb-12 text-center"
            data-testid="desserts-menu-title"
          >
            Фірмові десерти
          </h4>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-6xl mx-auto">
            {dessertsMenu.map((item, index) => (
              ...
              >
                ...
                <h5
                  className="text-lg font-serif font-semibold text-warm-brown mb-2"
                  data-testid={`dessert-name-${index}`}
                >
                  {item.name}
                </h5>
                <p
                  className="text-sm text-dark-text mb-4"
                  data-testid={`dessert-description-${index}`}
                >
                  {item.description}
                </p>
                <span
                  className="text-xl font-bold text-sage-green"
                  data-testid={`dessert-price-${index}`}
                >
                  ₴{item.price}
                </span>
              </div>
